...
//              println("FF:" + field.getName() + "(" + field.getType() + ")" + ",=>" + row(field.getName()) + ",type=" + row(field.getName()).getClass)
  ...
  def bean2SelectiveArray(bean: T): Seq[Any] = beanValue(bean, _.get(bean) != null)
  ...
  //  def findByCond(bean: T,range:Range = null)(implicit f: DBResult => Unit = null, noexec: Boolean = false): Future[QueryResult] = exec(SelectByCond(bean) + rangeStr(range), bean2SelectiveArray(bean))(f,noexec)
