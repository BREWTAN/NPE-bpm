...
object TestProcessCond {
  ...
  def main(args: Array[String]) {
    ...
    ProcDefHelper.appendDefFile("/Users/brew/NPE/workspace/npe/flows/testsplit.bpmn");
    ...
    for (i <- 1 to 1) {
      ...
      Cluster(system).registerOnMemberUp {
        ...
        cmd ! CMDSubmit(MessageHelper.wrappedANewProcess(UUID.randomUUID().toString(), Thread.currentThread().getName(), "test.diverge", 
            ContextData(0,0,null,0,Some(3))))
